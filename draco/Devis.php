...
elseif($PC->rcvG['action'] == 'cloner') {
    ...
    $info = new devisModel();
    $result = $info->getDataFromID($PC->rcvG['dev']);
    $id = $info->createId($result[1][0]['affaire_dev']);
    $prod=$info->getProduitsFromID($PC->rcvG['dev']);
...
elseif ($PC->rcvG['action'] == 'suppconfirm' and $PC->rcvG['id_devis'] != '') {
    ...
    $bddtmp->makeRequeteUpdate('devis',"id_dev",$PC->rcvG['id_devis'],array("status_dev"=>"2"));
    $bddtmp->process();
    $bddtmp->addActualite($PC->rcvG['id_devis'], 'delete');
...
elseif ($PC->rcvG['action'] == 'pdf') {
    $affaire = substr($PC->rcvG['id_devis'],0,6);
    $bddtmp->makeRequeteSelect('affaire','id_aff',$affaire);
    $aff = $bddtmp->process();
    $aff = $aff[0];
    $name = 'Devis.'.$PC->rcvG['id_devis'].'.pdf';
    $path = $GLOBALS['SVN_Pool1']['WorkCopy'].$GLOBALS['SVN_Pool1']['WorkDir'].$GLOBALS['ZunoAffaire']['dir.affaire'].$aff['dir_aff'];
...
elseif ($PC->rcvG['action'] == 'clone') {
	...
	$bddtmp->makeRequeteFree("SELECT * FROM affaire WHERE id_aff = '".$oldAffaire."'");
	$res = $bddtmp->process();
	if (count($res) > 0) {
	    $inputDB = $res[0];
	}
	...
	// on insert le devis
	$bddtmp->makeRequeteInsert('devis',$var_recv);
	$bddtmp->process();
	...
	if (count($res) > 0) {
		...
		// on insert le devis
		$bddtmp->makeRequeteInsert('devis_produit',$produit);
		$bddtmp->process();
	    }
	}
	// on insert l'actualité
	$bddtmp->addActualite($var_recv['id_dev'], 'clone','','',$id_dev);
	// On laisse une trace dans l'histo de l'affaire
	$bddtmp->makeRequeteUpdate('affaire',"id_aff",$affaire,array("status_aff"=>"3"));
	$bddtmp->process();
	$location = "draco/Devis.php?id_dev=".$var_recv['id_dev'];
	// si clone de renew, on mets a jour l'entrée de renouvelement
	if ($PC->rcvG["renew"] == "yes") {
	    $bddtmp->makeRequeteUpdate('devis_renew',"id_devis",$id_dev,array("devisgenere"=>$var_recv['id_dev']));
	    $bddtmp->process();
	}
    }
}
elseif($PC->rcvP['action'] == "Zieuter") {
    aiJeLeDroit('devis', 62, 'web');
    $Doc = 'Devis.'.$PC->rcvP["id_dev"].'.'.$PC->rcvP['format'];
