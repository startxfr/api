...
class commandeView {
        ...
        if (is_array($result) and count($result) > 0) {
            ...
            foreach ($result as $k => $v) {
                ...
                if ($letter != $tempmois || $annee != $tempannee) {
                    $letter = $tempmois;
                    $annee = $tempannee;
                    $list .= '</ul><h2>' . ucfirst(strftime("%B", strtotime('2008-' . $letter . '-27'))) . ' 20' . substr($v['id_cmd'], 0, 2) . '</h2><ul class="iArrow">';
                } elseif ($from != 0) {
    ...
    static function view($value = array(), $mode = '', $fourn = array()) {
        ...
        $somme = ($value['sommeHT_cmd'] != NULL) ? '<li>Total HT : <small>' . formatCurencyDisplay($value['sommeHT_cmd']) . '</small></li>' : '';
        $sommeFHT = ($value['sommeFHT_cmd'] != NULL) ? '<li>Total FHT : <small>' . formatCurencyDisplay($value['sommeFHT_cmd']) . '</small></li>' : '';
        $sommeTTC = ($value['sommeHT_cmd'] != NULL) ? '<li>Total TTC : <small>' . formatCurencyDisplay(($value['sommeHT_cmd'] * $txTva)) . '</small></li>' : '';
        ...
        $marge = '<li>Marge : <small>' . formatCurencyDisplay($value['sommeHT_cmd'] - $value['sommeFHT_cmd']) . '</small></li>';
...
//On vient d'effectuer tout un tas de tests pour s'assurer  que l'on ne vas afficher que des blocks avec des choses dedans.
        ...
        if ($value == NULL) {
            ...
            foreach ($value as $v) {
                ...
                if (round($v['prixF'], 0) != $v['prixF'])
                    $v['prixF'] = formatCurencyDisplay($v['prixF']);
                else
                    $v['prixF'] = formatCurencyDisplay($v['prixF'], 0);
                ...
                if (round($v['remiseF'], 0) != $v['remiseF'])
                    $v['remiseF'] = formatCurencyDisplay($v['remiseF'], 2, '%');
                else
                    $v['remiseF'] = formatCurencyDisplay($v['remiseF'], 0, '%');
                if (round($totalF, 0) != $totalF)
                    $totalF = formatCurencyDisplay($totalF);
                else
                    $totalF = formatCurencyDisplay($totalF, 0);
                ...
                $out .= $stock;
                $out .= '<li><label>Prix fournisseur : </label>' . $v['prixF'] . '</li>';
                $out .='<li><label>Remise Fournisseur : </label>' . $v['remiseF'] . '</li>';
                $out .='<li><label>Total Fournisseur : </label>' . $totalF . '</li></ul>';
                $out .='<ul><li><label>Remise Client : </label>' . $v['remise'] . '</li>';
                $out .='<li><label>Total Client : </label>' . $total . '</li>';
                $out .='</ul>';
                $out .='<ul><li><label>Marge : </label>' . formatCurencyDisplay($margeintermediaire) . '</li></ul></fieldset>';
            }
            ...
            $out .='</ul>';
            $out .='<ul><li><label>Total FHT : </label>' . formatCurencyDisplay($totalFHT) . '</li>';
            $out .='<li><label>Marge totale : </label>' . formatCurencyDisplay($marge) . '</li></ul></fieldset>';
            $produits .= $out;
        }
    ...
    static function blockProduitsModif($value = array(), $id_cmd = NULL, $onfékoi = 'rien') {
        ...
        $totalFourn = $value[0]['quantite'] * $value[0]['prixF'] * (1 - $value[0]['remiseF'] / 100);
        ...
        $totalF = '<li id="id_produitCtotalF">TT F : ' . formatCurencyDisplay($totalFourn) . '</li>';
        $totalV = '<li id="id_produitCtotalV">TT V : ' . formatCurencyDisplay($prixtotal) . '</li>';
    ...
    static function subBlockRessourcesLiees($value = array(), $fourn = array(), $facture = '') {
        ...
        if (file_exists($PathAff . 'PVRecette.' . $value['id_dev'] . '.pdf'))
            $outLi .= "<li><a href=\"../pegase/Commande.php?id_commande=" . $value['id_cmd'] . "&action=VoirPVR\" target=\"_blank\">" . imageTag(getStaticUrl('img') . 'prospec/commande.pdf.png', 'PDF') . ' PVRecette.' . $value['id_dev'] . ".pdf</a></li>";
    ...
    static function delete($value = array()) {
        ...
        if ($value['daterecord_cmd'] != NULL)
            $creation = strftime("%A %d %B %G", strtotime($value['daterecord_cmd']));
        $somme = ($value['sommeHT_cmd'] != NULL) ? '<li>Somme total HT : <small>' . formatCurencyDisplay($value['sommeHT_cmd']) . '</small></li>' : '';
        $entreprise = ($value['entreprise_cmd'] != NULL) ? '<li>Entreprise : ' . contactEntrepriseView::contactLinkSimple($value) . '</li>' : '';
        $contact = ($value['contact_cmd'] != NULL) ? '<li>Contact : ' . contactParticulierView::contactLinkSimple($value) . '</li>' : '';
    ...
    static function inputAjaxCommande($nom = '', $selected = '', $titre = '', $withBlank = true) {
        ...
        if ($selected != '') {
            ...
            if ($result[0]) {
                ...
                $nomSelected = '<i>Veuillez choisir une commande</i>';
            else
                $nomSelected = '&nbsp;';
        }
        elseif (!$withBlank)
...
<div class="iPanel">
    ...
    static function tri_montant($value = array(), $limit = 20, $from = 0, $total = 0) {
        ...
        foreach ($value[1] as $k => $v) {
            ...
            $prix = $sortie[1];
            $out .= $sortie[0];
            $out .= '<li><a href="Commande.php?action=view&id_cmd=' . $v['id_cmd'] . '"rev="async">' . $v['id_cmd'] . ' - ' . $v['titre_cmd'] . ' <small>Total Fournisseur : <b>' . formatCurencyDisplay($v['sommeFHT_cmd']) . '</b></small></a></li>';
        }
        if ($total > ($limit + $from))
    ...
    static function tri_creation($value = array(), $limit = 20, $from = 0, $total = 0) {
        ...
        foreach ($value[1] as $k => $v) {
            if ($v['daterecord_cmd'] != NULL) {
                if ($mois != ucfirst(strftime("%B %G", strtotime($v['daterecord_cmd'])))) {
                    $mois = ucfirst(strftime("%B %G", strtotime($v['daterecord_cmd'])));
