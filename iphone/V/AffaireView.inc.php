...
class affaireView
{
	...
	static function searchResult($result, $from = 0, $limit = 5, $total = 0, $qTag = '')
	{
		if(is_array($result) and count($result) > 0)
		{
			...
			foreach($result as $k => $v)
			{
				...
				if($letter != substr($v['id_aff'],2,2) || $annee != $tempannee) {$annee = $tempannee; $letter = substr($v['id_aff'],2,2) ; $list .= '</ul><h2>'.ucfirst(strftime("%B",strtotime('2008-'.$letter.'-27'))).' 20'.substr($v['id_aff'],0,2).'</h2><ul class="iArrow">'; }
	...
	static function view($value = array(),$mode = '')
	{
		...
		if($mode == 'afterModif') {$linkHead = '<a href="#_MainMenu"  rel="action" class="iButton iBAction"><img src="Img/home.png" alt="Accueil" /></a>';}
		else				  {$linkHead = '<a href="Affaire.php?action=modifAffaire&id_aff='.$value["id_aff"].'"  rev="async" rel="action" class="iButton iBAction"><img src="Img/edit.png" alt="Modifier" /></a>';}
 ...
 */
		...
		foreach($value[1] as $k => $v)
		{
			if($v['echeance_aff'] != NULL)
			{
				if(strtotime($v['echeance_aff']) <= time())
				{
					...
					if( $mois != 'expire' ) {$out .= '</ul><h2>Expirées</h2><ul class="iArrow">';}
					$mois = 'expire';
				...
				{
					if($mois != ucfirst(strftime("%B %G", strtotime($v['echeance_aff'])))) { $mois = ucfirst(strftime("%B %G", strtotime($v['echeance_aff']))); $out .= '</ul><h2>'.$mois.'</h2><ul class="iArrow">'; }
					$echeance=strftime("%d/%m/%G", strtotime($v['echeance_aff']));
					$out .= '<li><a href="Affaire.php?action=view&id_aff='.$v['id_aff'].'" rev="async">'.$v['id_aff'].' - '.$v['titre_aff'].' <small>Echéance : <b>'.$echeance.'</b></small></a></li>';
				}
			}
	...
	static function tri_creation($value = array(), $limit, $from, $total)
	{
		...
		foreach($value[1] as $k => $v)
		{
			if($v['detect_aff'] != NULL)
			{
					...
					if($mois != ucfirst(strftime("%B %G", strtotime($v['detect_aff'])))) { $mois = ucfirst(strftime("%B %G", strtotime($v['detect_aff']))); $out .= '</ul><h2>'.$mois.'</h2><ul class="iArrow">'; }
	...
	static function tri_modification($value = array(), $limit, $from, $total)
	{
		...
		foreach($value[1] as $k => $v)
		{
			if($v['modif_aff'] != NULL)
			{
					...
					if($mois != ucfirst(strftime("%B %G", strtotime($v['modif_aff'])))) { $mois = ucfirst(strftime("%B %G", strtotime($v['modif_aff']))); $out .= '</ul><h2>'.$mois.'</h2><ul class="iArrow">'; }
	...
	static function tri_nomAffaire($value = array(), $limit, $from, $total)
	{
		...
		foreach($value[1] as $k => $v)
		{
			if($v['nom_ent'] != NULL)
			{
					...
					if($ent != strtoupper($v['titre_aff']{0})) { $ent=strtoupper($v['titre_aff']{0}); $out .= '</ul><h2>'.$ent.'</h2><ul class="iArrow">'; }
					$entreprise=$v['nom_ent'];
					$out .= '<li><a href="Affaire.php?action=view&id_aff='.$v['id_aff'].'" rev="async">'.$v['id_aff'].' - '.$v['titre_aff'].' <small><b>'.$entreprise.'</b></small></a></li>';
	...
	static function result_avance ($value)
	{
		...
		{
			case 1:
					return '<div class="iList">'.self::tri_nomAffaire($value, 1, 1, 1).'</div>';break;
			case 2:
					return '<div class ="iList">'.self::tri_creation($value, 1, 1, 1).'</div>';break;
			case 3:
					return '<div class ="iList">'.self::tri_entreprise($value, 1, 1, 1).'</div>';break;
			case 4:
					return '<div class ="iList">'.self::tri_contact($value, 1, 1, 1).'</div>';break;
			case 5:
					return '<div class ="iList">'.self::tri_echeance($value, 1, 1, 1).'</div>';break;
			case 6:
					return '<div class ="iList">'.self::tri_creation($value, 1, 1, 1).'</div>';break;
			case 7:
					return '<div class="iList">'.self::tri_modification($value, 1, 1, 1).'</div>';break;
			default :
