...
class contactEntrepriseView
{
	...
	static function searchResult($result, $from = 0, $limit = 5, $total = 0, $qTag ='', $fourn = 'non')
	{
		if(is_array($result) and count($result) > 0)
		{
			...
			{
				if($letter != strtoupper($v['nom_ent']{0})) { $letter = strtoupper($v['nom_ent']{0}) ; $list .= '</ul><h2>'.$letter.'</h2><ul class="iArrow">'; }
...
class contactParticulierView
{
	...
	static function searchResult($result, $from = 0, $limit = 5, $total = 0, $qTag = '')
	{
		if(is_array($result) and count($result) > 0)
		{
			...
			foreach($result as $k => $v)
			{
				if($letter != $v['nom_cont']{0}) { $letter = strtoupper($v['nom_cont']{0}) ; $list .= '</ul><h2>'.$letter.'</h2><ul class="iArrow">'; }
				elseif($from != 0) {$list .= '</ul><ul class="iArrow">';}
