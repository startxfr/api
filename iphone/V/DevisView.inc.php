...
class devisView
{
	...
	static function searchResult($result, $from = 0, $limit = 5, $total = 0, $qTag = '')
	{
		if(is_array($result) and count($result) > 0)
		{
			...
			foreach($result as $k => $v)
			{
				...
				if($letter != $tempmois || $annee != $tempannee) { $annee = $tempannee; $letter = $tempmois ; $list .= '</ul><h2>'.ucfirst(strftime("%B",strtotime('2008-'.$letter.'-27'))).' 20'.substr($v['id_dev'],0,2).'</h2><ul class="iArrow">'; }
	...
	static function view($value = array(),$mode = '')
	{
		...
		$creation = '';
		$tva = '<li>T.V.A : <small>'.number_format($_SESSION['tva'],2,',',' ').' % ('.number_format(($value['sommeHT_dev']*$_SESSION['tva']/100),2,',',' ').' €)</small></li>';
		if($value['daterecord_dev'] != NULL)
			$creation = strftime("%A %d %B %G", strtotime($value['daterecord_dev']));
		$somme	 = ($value['sommeHT_dev'] != NULL) ? '<li>Total HT : <small>'.number_format($value['sommeHT_dev'],2,',',' ').' &euro;</small></li>' : '';
		$sommeTTC	 = ($value['sommeHT_dev'] != NULL) ? '<li>Total TTC : <small>'.number_format(($value['sommeHT_dev']*$txTva),2,',',' ').' &euro;</small></li>' : '';
		...
		if($mode == 'afterModif') {$linkHead = '<a href="Devis.php?action=view&id_dev='.$value['id_dev'].'"  rel="action" class="iButton iBAction"><img src="Img/config.png" alt="Recharger" /></a>';}
		else				  {$linkHead = '<a href="Devis.php?action=modifDevis&id_dev='.$value["id_dev"].'"  rev="async" rel="action" class="iButton iBAction"><img src="Img/edit.png" alt="Modifier" /></a>';}
		...
		$out = $linkHead.'<div class="iPanel">
				...
				</fieldset>'.self::subBlockRessourcesLiees($value, $commande, $facture).'
				<fieldset>
	...
	static function subBlockRessourcesLiees($value = array(), $commande = '', $facture = '')
	{
		...
		if (file_exists($GLOBALS['SVN_Pool1']['WorkCopy'].$GLOBALS['SVN_Pool1']['WorkDir'].$GLOBALS['ZunoAffaire']['dir.affaire'].$value['dir_aff'].$GLOBALS['ZunoDevis']['file.suffixe'].$value['id_dev'].'.pdf'))
			$outLi   .= "<li><a href=\"../draco/Devis.php?id_devis=".$value['id_dev']."&action=pdf\" target=\"_blank\">".imageTag('../img/prospec/devis.pdf.png','PDF').' '.$GLOBALS['ZunoDevis']['file.suffixe'].$value['id_dev'].".pdf</a></li>";
		...
		$out = '<fieldset>
					...
					<ul class="iArrow">
						<li><a rev="async" href="Actualite.php?action=viewDevis&amp;id_dev='.$value['id_dev'].'"><img src="Img/actualite.png"/> '.$totalActu.' Actualités</a></li>
						'.$commande.$facture.$outLi.'
					</ul>
				</fieldset>';
	...
	static function produits($value = array(), $id_dev, $prod = '' ,$mode = '')
	{
		...
		else
		{
			...
			foreach($value as $v)
			{
				...
				$out .='<li><label>Total : </label>'.$total.' &euro;</li>';
				$out .='</ul></fieldset>';
			}
			$TTC = (1+$TVA/100)*$totalHT;
			$out.='<fieldset><legend>Total de l\'offre '.$v['id_dev'].'</legend><ul class="iArrow">';
			$out .='<li><label>Total HT : </label>'.number_format($totalHT,2,',',' ').' &euro;</li>';
	...
	static function delete($value = array())
	{
		...
		if($requeteC[1][0]['total'] >=1 )
		{
			...
			else{$facture = '<strong> Cela supprimera également la commande '.$value['id_dev']."BC".' liée</strong>';}
		}
		$creation = '';
		if($value['daterecord_dev'] != NULL)
			$creation = strftime("%A %d %B %G", strtotime($value['daterecord_dev']));
		$somme	 = ($value['sommeHT_dev'] != NULL) ? '<li>Somme total HT : <small>'.number_format($value['sommeHT_dev'],2,',',' ').' &euro;</small></li>' : '';
		$entreprise	 = ($value['entreprise_dev'] != NULL) ? '<li>'.contactEntrepriseView::contactLinkSimple($value).'</li>' : '';
		$contact	 = ($value['contact_dev'] != NULL) ? '<li>'.contactParticulierView::contactLinkSimple($value).'</li>' : '';
		...
		$out = $linkHead.'<div class="iPanel">' .
				'<div class="err">
			  		<strong> Êtes vous sur de vouloir supprimer ce devis? </strong>'
					.$facture.'
				</div>
				<fieldset>
					<legend></legend>
	...
	static function inputAjaxProduit($nom = '', $selected = '', $titre = '', $withBlank = true, $commande = NULL, $de = '')
	{
		...
		if($selected != '')
		{
			...
			elseif(!$withBlank) $nomSelected = '<i>Veuillez choisir un produit</i>';
			else			  $nomSelected = '&nbsp;';
		}
		elseif(!$withBlank) $nomSelected = '<i>Veuillez choisir un produit</i>';
		else			  $nomSelected = '&nbsp;';
	...
	static function inputAjaxDevis($nom = '', $selected = '', $titre = '', $withBlank = true)
	{
		...
		if($selected != '')
		{
			...
			elseif(!$withBlank) $nomSelected = '<i>Veuillez choisir un devis</i>';
			else			  $nomSelected = '&nbsp;';
		}
		elseif(!$withBlank) $nomSelected = '<i>Veuillez choisir un devis</i>';
		else			  $nomSelected = '&nbsp;';
		...
		static function tri_montant($value = array(), $limit, $from, $total)
	{
		...
		foreach($value[1] as $k => $v)
		{
			...
			$prix = $sortie[1];
			$out .= $sortie[0];
			$out .= '<li><a href="Devis.php?action=view&id_dev='.$v['id_dev'].'"rev="async">'.$v['id_dev'].' - '.$v['titre_dev'].' <small>Montant : <b>'.number_format($v['sommeHT_dev'], 2, ',', ' ').' €</b></small></a></li>';
	...
	static function tri_creation($value = array(), $limit, $from, $total)
	{
		...
		foreach($value[1] as $k => $v)
		{
			if($v['daterecord_dev'] != NULL)
			{
					...
					if($mois != ucfirst(strftime("%B %G", strtotime($v['daterecord_dev'])))) { $mois = ucfirst(strftime("%B %G", strtotime($v['daterecord_dev']))); $out .= '</ul><h2>'.$mois.'</h2><ul class="iArrow">'; }
