...
elseif($PC->rcvG['action'] == 'validFormCP')
{
	...
	{
		$sqlConn->makeRequeteFree("select distinct CP, Ville, Pays from code_postal where Ville like '%".$PC->rcvP['valeur']."%' order by Ville, CP ;");
	}
...
elseif($PC->rcvG['action'] == 'validFormTVA')
{
	...
	if(!is_numeric($ttc))
	{
		...
		$parametres = array('Prix TTC : ', number_format($result, 2, ',', ' '), ' €');
	}
	elseif(!is_numeric($ht))
	{
		$result = $ttc/(1+$PC->rcvP['tva']);
		$parametres = array('Prix HT : ', number_format($result, 2, ',', ' '), ' €');
	}
	else
	{
		$result = ($ttc/$ht-1)*100;
		$parametres = array('TVA : ', number_format($result, 2, ',', ' '), ' %');
	}
