...
elseif($PC->rcvG['action'] == 'inputCommandeContinue') {
    if(verifDroits('commande',10)) {
	$plus = '';
	else $plus = "AND commercial_cmd = '".$_SESSION['user']['id']."' ";
	$zoneTo = $outJs = $out = '';
	$limit = $_SESSION['user']['config']['LenghtSearchCommande'];
...
<root>
	...
	if (($PC->rcvG['action'] == 'doRec')or($PC->rcvG['action'] == 'doRecsend')) {
	    ...
	    else $actuDesc = 'Le document '.substr($mess,0,-2).' de la commande '.$dev['id_cmd'].' vient d\'être généré. Il est disponible dans le disque Dur Virtuel. Commentaire de l\'enregistrement : '.$PC->rcvP['message'];
