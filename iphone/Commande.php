...
elseif($PC->rcvG['action'] == 'inputCommandeResult') {
    ...
    else	$out .= '<h2 class="Contact">Commande (0)</h2>';
...
elseif($PC->rcvG['action'] == 'send1') {
    ...
    elseif($PC->rcvP['type'] == 'fax') {
        ...
        else  $_SESSION['CommandeActionRecSend']['fax'] = $r['fax_achat'];
    }
    else {
        if($result[1][0]['commercial_cmd'] != $_SESSION['user']['id']) {
        ...
        else  $_SESSION['CommandeActionRecSend']['email'] = $r['mail_achat'];
...
elseif (($PC->rcvG['action'] == 'doVoir')or
    ...
    if ($PC->rcvG['action'] == 'doRecsend') $PC->rcvP = array_merge($_SESSION['CommandeActionRecSend'],$PC->rcvP);
...
<root>
    ...
    if (($PC->rcvG['action'] == 'doRec')or($PC->rcvG['action'] == 'doRecsend')) {
        ...
        $inActualiteRec = array(
                'type' => 'commande',
                'titre' => $actuTitre,
                'desc' => $actuDesc,
                'date' => date('y-m-d H:i:s'),
                'user' => $_SESSION['user']['id'],
                'id_ent' => $dev['entreprise_cmd'],
                'id_cont' => $dev['contact_cmd'],
                'id_aff' => substr($dev['id_cmd'],0,6),
                'id_cmd' => $dev['id_cmd'],
                'status_cmd' => '3');
    ...
    if (($PC->rcvG['action'] == 'doRecsend')or($PC->rcvG['action'] == 'doSend')) {
        ...
        if($control[0]) {
            ...
            if($result[0]) {
                ...
                $inActualiteEnvoi = array(
                        'type' => 'commande',
                        'titre' => $actuPrefix.'nvoi de la commande '.$dev['id_cmd'],
                        'desc' => 'La commande '.$dev['id_cmd'].' vient d\'être '.strtolower($actuPrefix).'nvoyée par '.$PC->rcvP['type'].' à '.$dest,
                        'date' => date('y-m-d H:i:s'),
                        'user' => $_SESSION['user']['id'],
                        'id_ent' => $dev['entreprise_cmd'],
                        'id_cont' => $dev['contact_cmd'],
...
<root><go to="waCommandeAction"/>
    <part><destination mode="replace" zone="waCommandeAction"/>
        <data><![CDATA[ <div class="msg">Votre document vient d'être envoyé par <?php echo $PC->rcvP['type']; ?>. </div> ]]></data>
            ...
            else { ?><root><go to="waCommandeAction1"/>
...
elseif($PC->rcvG['action'] == 'recep') {
    ...
    if($result[0]) {
        $inActualite = array(
                'type' => 'commande',
                'titre' => 'Commande : '.$donnee[1][0]['id_cmd'].' réceptionnée',
                'desc' => 'La commande '.$PC->rcvG['id_cmd'].' vient d\'être réceptionnée.',
                'date' => date('y-m-d H:i:s'),
                'user' => $_SESSION['user']['id'],
                'id_ent' => $donnee[1][0]['entreprise_fact'],
                'id_cont' => $donnee[1][0]['contact_fact'],
