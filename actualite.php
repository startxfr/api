...
if($PC->rcvG['action'] == 'ajax') {
   ...
   $suffixe = getStaticUrl('img');
    if($result[0]) {
	foreach($result[1] as $k => $v)
	    $sortie .= '<p><a href="#" onclick="return zuno.filActu.popupActu('.$v['id'].')"><img src=\''.$suffixe.'actualite/'.strtolower($v['type']).'.png\' alt=\''.$v['type'].'\'/> '.str_replace("\n", ' ',$v['titre']).'</a></p>';
    }
...
elseif($PC->rcvG['id'] != '') {
   ...
   $suffixe = getStaticUrl('img');
	...
	$contenu .= '<div class="field"><img src=\''.$suffixe.'actualite/'.strtolower($result[1][0]['type']).'.png\' alt=\''.$result[1][0]['type'].'\'/> &nbsp;'.$result[1][0]['type'].'</div></div>';
	...
	if($result[1][0]['id_ent'] != '') {
		...
		$result[1][0]['nom_ent']	= imageTag($suffixe.$GLOBALS['PropsecConf']['dir.img'].'TypeEntreprise/'.$result[1][0]['type_ent'].'.png',$result[1][0]['nom_tyent']).' '.$result[1][0]['nom_ent'];
	    else $result[1][0]['nom_ent']	= imageTag($suffixe.$GLOBALS['PropsecConf']['dir.img'].'TypeEntreprise/particulier.png','particulier').' <i>Particulier</i>';
	...
	if($result[1][0]['id_cont'] != '') {
	    ...
	    $contenu .= '<div class="field"><a href="'.$prefixe.'prospec/Contact.php?id_cont='.$result[1][0]['id_cont'].'"><img src="'.$suffixe.'actualite/contact.png" title="Contact", alt="contact" /> '.$result[1][0]['civ_cont'].' '.$result[1][0]['prenom_cont'].' '.$result[1][0]['nom_cont'].'</a></div></div>';
	}
	if($result[1][0]['id_aff'] != '') {
	    $contenu .= '<div class="row"><div class="label">Affaire : </div>';
	    $contenu .= '<div class="field"><a href="'.$prefixe.'draco/Affaire.php?id_aff='.$result[1][0]['id_aff'].'"><img src="'.$suffixe.'actualite/affaire.png" title="Affaire", alt="affaire" /> '.$result[1][0]['id_aff'].' - '.$result[1][0]['titre_aff'].' </a></div></div>';
	}
	if($result[1][0]['id_dev'] != '') {
	    $contenu .= '<div class="row"><div class="label">Devis : </div>';
	    $contenu .= '<div class="field"><a href="'.$prefixe.'draco/Devis.php?id_dev='.$result[1][0]['id_dev'].'"><img src="'.$suffixe.'actualite/devis.png" title="Devis", alt="devis" /> '.$result[1][0]['id_dev'].' - '.$result[1][0]['titre_dev'].'</a></div></div>';
	}
	if($result[1][0]['id_cmd'] != '') {
	    $contenu .= '<div class="row"><div class="label">Commande : </div>';
	    $contenu .= '<div class="field"><a href="'.$prefixe.'pegase/Commande.php?id_cmd='.$result[1][0]['id_cmd'].'"><img src="'.$suffixe.'actualite/commande.png" title="Commande", alt="commande"> '.$result[1][0]['id_cmd'].' - '.$result[1][0]['titre_cmd'].'</a></div></div>';
	}
	if($result[1][0]['id_fact'] != '') {
	    $contenu .= '<div class="row"><div class="label">Facture : </div>';
	    $contenu .= '<div class="field"><a href="'.$prefixe.'facturier/Facture.php?id_fact='.$result[1][0]['id_fact'].'"><img src="'.$suffixe.'actualite/facture.png" title="Facture", alt="facture"> '.$result[1][0]['id_fact'].' - '.$result[1][0]['titre_fact'].'</a></div></div>';
	}
	$contenu .= '</div></div>';
