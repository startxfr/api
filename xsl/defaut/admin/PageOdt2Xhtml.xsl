...
<xsl:template match="draw:frame">
	<div class="{translate(@draw:style-name,'.','_')}">
		<xsl:apply-templates/>
	</div>
</xsl:template>
<xsl:template match="draw:image">
	<xsl:param name="PhotoStock" select="$PhotoStock" />
	
	<img src="{$PhotoStock}{substring-after(@xlink:href,'ictures/')}"/>
</xsl:template>
<!--
