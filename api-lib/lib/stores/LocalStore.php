...
class LocalStore extends DefaultStore implements IStorage {
    ...
    public function init() {
        ...
        if($this->isConfig('dateformat'))
            $this->setConfig('dateformat', 'Y-m-d');
        if($this->isConfig('file'))
            $this->getConfig('file', 'default.txt');
        return $this;
    }

    public function connect() {
        $mode = ($this->getConfig('mode') == 'append') ? 'a+b' : 'r+b';
        $this->filename = sprintf($this->getConfig('file'), date($this->getConfig('dateformat')));
        $this->fullPath = $this->getConfig('path') . DS . $this->filename;
        if(!@file_exists($this->fullPath)) {
            if(!@touch($this->fullPath))
                throw new StoreException("we could not create file " . $this->filename . " into " . $this->getConfig('path') . DS);
        }
        $this->fileHandler = @fopen($this->fullPath, $mode);
        if($this->fileHandler === false)
            throw new StoreException('we could not open file ' . $this->filename . ' in directory ' . $this->getConfig('path') . DS);
        return $this;
    }
    ...
    protected function recordFile($message) {
        if(fwrite($this->fileHandler, $message) === false)
            throw new StoreException('we could not write into file ' . $this->filename . ' located in ' . $this->fullPath);
        return $this;
    }
