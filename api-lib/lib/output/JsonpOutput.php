 ...
 */
class JsonpOutput extends DefaultOutput implements IOutput {

    public function __construct($config) {
        parent::__construct($config);
        $callbackfct = (array_key_exists('callback', $config)) ? $config["callback"] : 'callback';
        $api = Api::getInstance();
        $input = $api->getInput();
        $this->callback = $input->getParam($callbackfct, 'callback');
    }
    ...
    protected function render($content, $httpCode = 200) {
        ...
        header('Content-Type: ' . $this->getConfig("content_type", "text/javascript") . '; charset=utf8');
