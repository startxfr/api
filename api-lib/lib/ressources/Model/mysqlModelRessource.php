...
/**
 * This ressource is used to interact (read - write) with mysql data, recorded in a store.
 ...
 */
class mysqlModelRessource extends defaultModelRessource implements IRessource {

    public function init() {
        parent::init();
        if (!is_object($this->model) or get_class($this->model) != 'mysqlModel')
            throw new RessourceException("Could not " . __FUNCTION__ . " " . get_class($this) . " because the provided model is not of type mysqlModel", 908);
        return $this;
    }
