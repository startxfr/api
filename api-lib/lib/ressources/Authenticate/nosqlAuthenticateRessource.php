...
/**
 * This ressource is used to authenticate a user by using a nosql database backend.
 *
 * @package  SXAPI.Resource.Authenticate
 * @author   Dev Team <dev@startx.fr>
 * @see      defaultAuthenticateRessource
 * @link     https://github.com/startxfr/sxapi/wiki/Resource
 */
class nosqlAuthenticateRessource extends defaultAuthenticateRessource implements IRessource {
    ...
    private function doAuthenticate($login, $pass) {
        ...
        if (is_array($data) and $data[$this->getConfig('id_field', "_id")] == $login) {
            $api->logInfo(960, "User '" . $login . "' authenticated with '" . get_class($this) . "'", $this->getRessourceTrace(__FUNCTION__, false), 1);
