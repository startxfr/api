...
class Session {
     ...
     */
    function RedirectSession($type = '') {
        if($type == 'TIME_OUT') {
            $this->Deconnect();
            $to = "Login.php?mess=timeout";
        }
        elseif($type == 'USER_ENDED') {
            $this->Deconnect();
            $to = "Login.php?mess=close";
        }
        elseif(($type == 'NO_SESSION_STORED')or
                ($type == 'NO_SESSION')or
                ($type == 'CORRUPTED_ID')or
                ($type == 'SESSION_NOT_SX'))
            $to = "Login.php?mess=badsess";
        elseif($type == 'BAD_RIGHT')
            $to = "Login.php?mess=badright";
        elseif($type == 'NO_USER_SESSION') {
            $this->Deconnect();
            $to = "Login.php?mess=nousersess";
        }
