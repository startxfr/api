...
class Bdd_mysql_2 {
	...
	$this->requete = $top.$head." WHERE ".$col_id." = '".addslashes($id)."' ".$autre;
	return $this->requete;
    }
	...
	if(is_array($liste)) {
		...
		if ($val{0} == '`')
		    $lescrit   .= " `".$key."` = ".$val."` AND";
		else  $lescrit   .= " `".$key."` = '".addslashes($val)."' AND";
	    }
	...
	if (!$resultat) {
	    if($log)
		Logg::loggerError('Bdd_mysql_2::process() ~ erreur SQL '.$this->requete,mysql_error(),__FILE__.'@'.__LINE__);
	...
	if (!$resultat) {
	    if($log)
		Logg::loggerError('Bdd_mysql_2::process2() ~ erreur SQL '.$this->requete,mysql_error(),__FILE__.'@'.__LINE__);
